...
router.get('/getGeneros', async (req, res) => {
  try{
    var generos = await generalService.getGenders();
    if(generos.error == 1){
      res.status(201).json({
        "data": [],
        "code_status": 400,
    ...
    console.log({
      "function": "prueba de trabajo: getGeneros",
      "date": fechaAhora,
      "errors": "Error en el servicio. Comuníquece con Servicio al Cliente.",
      "details": error
    });
...
router.post('/altaEstudiante', validatorHandler(estudianteSchema, 'params'), jsonParser, async (req, res) => {
  try{
    var alta = await generalService.addStudent(req.body);
    if(alta.num_error != 0){
      res.status(201).json({
        "data": "",
        "code_status": 400,
